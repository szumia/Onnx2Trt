...
//TODO::change your classes num
static const int NUM_CLASSES = 80;  //类别数
...
//TODO:: put your classes string vector here
//faces////
...
int main(int argc, char** argv)
 {
    ...
    if (file.good()) {
        std::cout<<" load engine success...   enigne path:"<<engine_file_path<<std::endl;
    ...
    {
        std::cerr<<"ERROD:: load engine failed...   enigne path:"<<engine_file_path<<std::endl;
        std::exit(-1);
    }
    ...
    //det模型trt初始化
    std::cout<<"start init det model trt..."<<std::endl;
    ...
    ICudaEngine* engine_det = runtime_det->deserializeCudaEngine(trtModelStreamDet, size);
     std::cout<<"Stop 126"<<std::endl;
    assert(engine_det != nullptr);
    //TODO::ERROR::Segmentation fault (core dumped) 下一行代码
    IExecutionContext* context_det = engine_det->createExecutionContext();
     std::cout<<"Stop 128"<<std::endl;
    assert(context_det != nullptr);
    delete[] trtModelStreamDet;
     std::cout<<"finish init det model trt..."<<std::endl;
    ...
    for (auto &input_image_path:imagList) 
    {
         ...
         for (int i = 0; i<count;i++)
        {
         ...
         std::cout<<"boxes count:"<<boxes_count<<std::endl;
         for (int i = 0; i<boxes_count; i++)
        {
            ...
            //fix next   line
            std::string  label_string = labels_name[(int)boxes[i].label]+" "+std::to_string(boxes[i].score);
            std::cout<<"box "<<i<<" label: "<<labels_name[(int)boxes[i].label]<<std::endl;
//            std::string  label_string = std::to_string((int)boxes[i].label)+" "+std::to_string(boxes[i].score);
            cv::putText(img, label_string, cv::Point(boxes[i].x1, boxes[i].y1 - 1), cv::FONT_HERSHEY_PLAIN, 1.2, cv::Scalar(0, 0, 255), 1.85);
        }
        ...
        std::cout<<"saved path: "<<image_name<<std::endl;
...
//TODO::$ execution engine_file_path img_path